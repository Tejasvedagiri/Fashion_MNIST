...
def Import_data_set():
    #Importing Data Set
    DATA_LOC = "Data/"
    Training_data_LOC = "train.csv"
    Training_data = pd.read_csv(DATA_LOC+Training_data_LOC)
    Test_data_LOC = "test.csv"
    Test_data = pd.read_csv(DATA_LOC+Test_data_LOC)
    
    #Loading X and y
    X_Train = Training_data.iloc[:,1:].values
    Y_Train = Training_data.iloc[:,0:1].values
    
    X_Pred = Test_data.iloc[:,1:].values
    Y_Pred_Actual = Test_data.iloc[:,0:1].values
